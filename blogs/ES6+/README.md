...
![export各个浏览器兼容性一览表](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/5/22/1638599b53c8a48d~tplv-t2oaga2asx-watermark.awebp)



由上图可知，有些浏览器对于ES6并不是很友好，针对 ES6 的兼容性问题，很多团队为此开发出了多种语法解析转换工具(比如babel，jsx，traceur 等)，可以把我们写的 ES6 语法转换成 ES5，相当于在 ES6 和浏览器之间做了一个翻译官。其中[Babel](https://link.juejin.cn?target=https%3A%2F%2Fbabeljs.io%2F)是一个广泛使用的转码器，可以将ES6代码转为ES5代码，从而在现有环境执行。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/5/22/1638599b540a14c4~tplv-t2oaga2asx-watermark.awebp)
...
**1.不再需要立即执行的函数表达式(IIFE)** 在 ES5 中，我们需要构造一个立即执行的函数表达式去保证我们不污染全局作用域。在 ES6中， 我们可以使用更简单的大括号（{}），然后使用 const 或者 let 代替 var 来达到同样的效果。

**2.循环体中的闭包不再有问题** 在 ES5 中，如果循环体内有产生一个闭包，访问闭包外的变量，会产生问题。在 ES6，你可以使用 “let” 来避免问题。

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/5/22/1638599b5402848f~tplv-t2oaga2asx-watermark.awebp)



**3.防止重复声明变量** ES6 不允许在同一个作用域内用 let 或 const 重复声明同名变量。这对于防止在不同的 js 库中存在重复声明的函数表达式十分有帮助。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/11/27/16752ef8c3725655~tplv-t2oaga2asx-watermark.awebp)
...
teamSummary函数里面又嵌了个函数，这导致内部的this的指向发生了错乱。 **那如何修改：**
...
**rest 参数搭配的变量是一个数组，该变量将多余的参数放入数组中。** 我们举个例子：如何实现一个求和函数？
...
**嵌套对象解构：** 使用类似于对象字面量的语法，可以深入到嵌套的对象结构中去提取你想要的数据。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/6/24/164323d8966b63a1~tplv-t2oaga2asx-watermark.awebp)
...
![回调地狱](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/5/22/1638599b54398625~tplv-t2oaga2asx-watermark.awebp)
...
![Promise原理](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/5/22/1638599b541da506~tplv-t2oaga2asx-watermark.awebp)
